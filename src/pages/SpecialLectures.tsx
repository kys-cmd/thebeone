...
export default function SpecialLectures() {
  ...
  const loadData = async () => {
    try {
      const [courseData, bannerData] = await Promise.all([
        ...
        cmsService.getBanners('special')
      ]);
      
      const filteredCourses = courseData.filter(c => c.category === 'special' || c.category === 'special_online' || c.category === 'special_offline');
  ...
  const onlineCourses = courses.filter(c => c.category === 'special_online' || (c.category === 'special' && (!c.location || c.location === '')));
  const offlineCourses = courses.filter(c => c.category === 'special_offline' || (c.category === 'special' && (c.location && c.location !== '')));

  return (
    <div className="min-h-screen bg-white pb-32">
       {/* Special Lectures Full-Width Rolling Banner (Hero) */}
       <section className="relative h-[500px] lg:h-[600px] w-full overflow-hidden bg-[#0a0a0a]">
        {!loading && banners.length > 0 ? (
          <>
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {banners[currentBannerIndex].video_url ? (
                  <div className="absolute inset-0 w-full h-full z-0 overflow-hidden">
                    {banners[currentBannerIndex].video_url.includes('vimeo.com') ? (
                      <iframe
                        src={getVimeoEmbedUrl(banners[currentBannerIndex].video_url, {
                          autoplay: 1,
                          loop: 1,
                          background: 1,
                          autopause: 0,
                      ...
                      <video 
                        src={banners[currentBannerIndex].video_url} 
                  ...
                  <img 
                    ...
                    className="w-full h-full object-cover transition-transform duration-10000 scale-105"
                ...
                <div className="container mx-auto px-4 h-full flex flex-col justify-center relative z-20">
                  <div className="max-w-4xl w-full flex flex-col justify-center">
                    {/* Standard Title/Subtitle Rendering */}
                    <div className="space-y-6">
                      <motion.div
                        ...
                        className="space-y-6"
                      >
                        <Badge className="bg-red-600 text-white border-none py-1.5 px-4 text-xs font-bold rounded-full w-fit animate-pulse">
                          LIMITED SPECIAL
                        </Badge>
                        <h1 className="text-3xl md:text-5xl lg:text-7xl font-black tracking-tighter text-white leading-[1.1]">
                          {banners[currentBannerIndex].title}
                        </h1>
                        {banners[currentBannerIndex].subtitle && (
                          <p className="text-base md:text-lg text-white/70 font-medium whitespace-pre-line leading-relaxed max-w-md">
                            {banners[currentBannerIndex].subtitle}
                          </p>
                        )}
                        
                        {/* Render HTML content if present beneath standard title/subtitle or as standalone */}
              ...
              {banners.map((_, idx) => (
                ...
                >
                  <div className={cn(
                    ...
                    currentBannerIndex === idx ? "w-10 bg-white" : "w-1.5 bg-white/30 group-hover:bg-white/50"
                  )} />
                </button>
              ))}
            </div>
          ...
          <div className="h-full flex items-center justify-center bg-[#0a0a0a] relative overflow-hidden">
             ...
             <div className="relative z-10 text-center">
                <h1 className="text-4xl font-black text-white">온라인/오프라인 특강</h1>
      ...
      <div className="container mx-auto px-4 space-y-32 py-20">
        ...
        <section className="space-y-12">
          <div className="flex items-end justify-between border-b-4 border-purple-600 pb-6">
            <div className="space-y-2">
              <h2 className="text-4xl font-black text-gray-900 tracking-tighter">온라인 특강</h2>
            </div>
          </div>
        ...
        <section className="space-y-12">
          <div className="flex items-end justify-between border-b-4 border-gray-900 pb-6">
            <div className="space-y-2">
              <h2 className="text-4xl font-black text-gray-900 tracking-tighter">오프라인 특강</h2>
            </div>
          </div>
...
function EmptyState({ icon }: { icon: React.ReactNode }) {
  return (
    <div className="col-span-full py-20 text-center space-y-4 border-2 border-dashed border-gray-50 rounded-[40px]">
      ...
      <p className="text-gray-500 font-bold">현재 진행 예정인 특강이 없습니다.</p>
    </div>
  );
}
